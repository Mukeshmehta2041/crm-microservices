...
public class GraphQLController {
  ...
  @Operation(summary = "Execute GraphQL query", description = "Execute a GraphQL query, mutation, or subscription")
  @ApiResponses(value = {
      @ApiResponse(responseCode = "200", description = "GraphQL query executed successfully", content = @Content(schema = @Schema(implementation = GraphQLResponse.class))),
  ...
  @Operation(summary = "Execute GraphQL query via GET", description = "Execute a simple GraphQL query via GET request")
  ...
  private Object buildResponse(ExecutionResult executionResult) {
    ...
    if (executionResult.getExtensions() != null && !executionResult.getExtensions().isEmpty()) {
      ...
      executionResult.getExtensions().forEach((key, value) -> extensions.put(key.toString(), value));
