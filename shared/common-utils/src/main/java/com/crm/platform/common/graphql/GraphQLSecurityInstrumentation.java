...
public class GraphQLSecurityInstrumentation extends SimpleInstrumentation {
    ...
    public DataFetcher<?> instrumentDataFetcher(DataFetcher<?> dataFetcher, InstrumentationFieldFetchParameters parameters) {
        return environment -> {
            ...
            String typeName = parameters.getFieldContainer().getName();
            
            if (isRestrictedField(typeName, fieldName)) {
                Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
                if (!hasFieldPermission(authentication, typeName, fieldName)) {
                    throw new AccessDeniedException("Insufficient permissions for field: " + typeName + "." + fieldName);
                }
            }
